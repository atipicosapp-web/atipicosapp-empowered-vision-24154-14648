...
serve(async (req) => {
  ...
  try {
    ...
    // Check rate limit
    const rateLimitCheck = await checkRateLimit(supabaseAdmin, user.id, 'create-product-checkout');
    ...
    const lineItems = items.map((item: any) => ({
      price_data: {
        ...
        product_data: {
          ...
          description: item.product.node.description || undefined,
        ...
        unit_amount: Math.round(parseFloat(item.price.amount) * 100), // Convert to cents
      },
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/?shop_success=true`,
      cancel_url: `${req.headers.get("origin")}/?shop_canceled=true`,
